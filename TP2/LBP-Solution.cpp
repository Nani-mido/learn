...
int main(int argc, const char *argv[]) {
    ...
    cout<<"For Image chose 0"<<endl;
    cout<<"For Video chose 1"<<endl;

    cin>>choice;

    switch(choice){
    case 0: // cas du TP avec une image

{
        ...
        lbp::OLBP(dst, lbp); // use the original operato
        ...
        Mat lbp_hist = lbp::histogram(lbp, 256);
        Mat lbp_spatial_hist = lbp::spatial_histogram(lbp, 256);

        // pour la distance entre deux histogram
        // double distance_h1h2 = chi_square(const Mat& histogram0, const Mat& histogram1);

        waitKey(0);
}// fin du sa du TP
        break;

    case 1: // cas optionnel de la webcam
        int deviceId = 0;
        if(argc > 1)
